...
**ATS Evaluator** (`/`) — Upload a PDF or DOCX resume and receive a structured compatibility report generated by Claude: ATS score, found/missing keywords, formatting issues, and actionable recommendations.

**Job Board** (`/jobs.html`) — Browse 100 remote tech jobs pulled from [Remotive](https://remotive.com). Upload your CV to:
...
│
├── src/static/
│   ├── index.html / app.js      # ATS Evaluator page
│   ├── style.css                # Global styles (CSS vars, Steam color palette)
...
The job board uses a **glassmorphism** aesthetic with the Steam color palette:
